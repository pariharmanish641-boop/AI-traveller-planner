...
import { 
  ...
  Search, UserCheck, User, MessageSquare, X, ExternalLink, Download, LogIn
} from 'lucide-react';
...
export default function App() {
  ...
  const generateTrip = async () => {
    ...
    if (!activeKey) {
      alert("Please set your Groq API Key in top bar or .env file!");
...
- Estimated Cost Breakdown (Travel, Food, Sightseeing)`;
  ...
  // Chatbot Handler using Groq API
  const handleSendMessage = async () => {
    ...
    if (!activeKey) {
      setChatMessages([...newMessages, { sender: 'bot', text: 'Please set your Groq API Key to chat.' }]);
      return;
    }
    ...
    try {
      const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
        ...
        body: JSON.stringify({
          model: "llama-3.3-70b-versatile",
          messages: [{ role: "user", content: `Answer as a helpful travel assistant concisely: ${query}` }]
        })
      });
      ...
      setChatMessages([...newMessages, { sender: 'bot', text: 'Sorry, issue connecting to Groq AI.' }]);
    }
  };
      ...
      <main className="max-w-6xl mx-auto mt-8 px-4">
        <div className="bg-white rounded-2xl p-6 lg:p-8 shadow-md border border-slate-200">
          ...
          <button 
            ...
            disabled={loading}
            className="w-full bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white font-extrabold text-lg py-4 rounded-xl transition shadow-lg flex items-center justify-center gap-2 disabled:opacity-50 cursor-pointer"
          >
        ...
        {itinerary && (
          <div className="mt-10 bg-white rounded-2xl p-6 lg:p-8 shadow-lg border border-slate-200">
            <div className="flex justify-between items-center mb-6 pb-4 border-b border-slate-100">
              ...
              <button 
                onClick={exportPDF} 
                className="bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold px-4 py-2.5 rounded-xl flex items-center gap-2 transition cursor-pointer"
              >
                <Download className="w-4 h-4" /> Download PDF
              </button>
            </div>

            <div id="itinerary-content" className="prose max-w-none text-slate-700 leading-relaxed whitespace-pre-wrap font-sans text-sm lg:text-base">
              {itinerary}
            </div>
          </div>
        )}
      ...
      <div className="fixed bottom-6 right-6 z-50">
        {!chatOpen ? (
          <button 
            ...
            className="bg-orange-500 hover:bg-orange-600 text-white p-4 rounded-full shadow-2xl transition transform hover:scale-110 flex items-center gap-2 font-bold cursor-pointer"
          >
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-80 sm:w-96 h-[480px] flex flex-col overflow-hidden">
            <div className="bg-orange-500 text-white p-4 flex justify-between items-center">
              <div className="flex items-center gap-2 font-bold">
                <MessageSquare className="w-5 h-5" /> AI Travel Assistant
              </div>
              <button onClick={() => setChatOpen(false)} className="cursor-pointer"><X className="w-5 h-5" /></button>
            </div>
            ...
            <div className="p-3 bg-white border-t flex gap-2">
              ...
              <button 
                ...
                className="bg-orange-500 text-white px-3 py-2 rounded-xl text-xs font-bold cursor-pointer"
              >
                Send
              </button>
            </div>
      ...
      {showLoginModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl relative">
            <button onClick={() => setShowLoginModal(false)} className="absolute top-4 right-4 text-slate-400 hover:text-slate-600 cursor-pointer">
              <X className="w-5 h-5" />
            </button>
            ...
            <button 
              ...
              className="w-full border border-slate-300 hover:bg-slate-50 text-slate-700 font-semibold py-3 rounded-xl flex items-center justify-center gap-2 text-sm transition cursor-pointer mb-3"
            >
